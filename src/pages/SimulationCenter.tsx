    ...
    <div className="max-w-5xl mx-auto space-y-12 pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-2">Test PULSEPATH response to various match-day scenarios.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-[1fr_350px] gap-12">
        <div className="space-y-8">
          <h2 className="text-xl font-bold uppercase tracking-widest text-gray-500">Select Scenario</h2>
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  <div className={cn(
                    ...
                    state.scenario === scenario.id ? "bg-[#ccff00] text-black" : "bg-white/5 text-gray-400"
                  )}>
                    {getScenarioIcon(scenario.id)}
                  </div>
                  {state.scenario === scenario.id && (
                    <div className="text-[10px] font-black uppercase text-[#ccff00] px-2 py-0.5 border border-[#ccff00]/50 rounded">Active</div>
                  )}
                </div>
        ...
        <div className="space-y-8">
          <h2 className="text-xl font-bold uppercase tracking-widest text-gray-500">Environment Controls</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {['sunny', 'rainy', 'windy', 'stormy'].map(cond => (
                  <button 
                    key={cond}
                    onClick={() => updateState({ weather: { ...state.weather, condition: cond as any } })}
                    className={cn(
...
function getScenarioIcon(id: string) {
  switch(id) {
    case 'normal-arrival': return <Users className="w-5 h-5" />;
    case 'heavy-rain': return <CloudRain className="w-5 h-5" />;
    case 'transit-delay': return <Bus className="w-5 h-5" />;
    case 'gate-closure': return <ShieldAlert className="w-5 h-5" />;
    case 'exit-surge': return <Timer className="w-5 h-5" />;
