  ...
  const handlePlanRoute = async (destLabel?: string) => {
    ...
    try {
      ...
      setActiveRoute({
        ...
        distance: '450m',
        congestion: 'Low',
        reason: 'Gate C is currently 41% less congested than Gate A. Rerouting via Zone 3 avoids the main concourse surge.'
      });
    }
  };
      ...
      <div className="flex flex-col gap-6 overflow-y-auto pr-2">
        {!activeRoute ? (
          ...
          >
            ...
            <p className="text-gray-500 text-sm mb-8">Select a destination to generate an AI-optimized route.</p>
            
            <div className="space-y-4">
              <QuickDest label="Gate A (Assigned)" time="12m" density="High" onClick={() => handlePlanRoute('Gate A (Assigned)')} />
              <QuickDest label="Gate C (Recommended)" time="8m" density="Low" recommended onClick={() => handlePlanRoute('Gate C (Recommended)')} />
              <QuickDest label="Food Court East" time="5m" density="Med" onClick={() => handlePlanRoute('Food Court East')} />
              <QuickDest label="Accessibility Hub" time="3m" density="Low" onClick={() => handlePlanRoute('Accessibility Hub')} />
            </div>
          ...
          >
            ...
            <div className="bg-[#ccff00] text-black rounded-3xl p-8">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-60">AI Route Active</div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-4">
              <div className="text-xs font-bold text-gray-500 uppercase tracking-widest px-2">Navigation Steps</div>
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-6">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">
            PULSEPATH is monitoring real-time flow through Gates A-D. Heavy congestion detected in North Concourse due to pre-match security screening.
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="flex items-center gap-3 text-[10px] text-gray-500 font-bold uppercase tracking-widest">
          ...
          <span className="flex items-center gap-1.5">
            <div className={cn(
              ...
              density === 'Low' ? 'bg-[#ccff00]' : density === 'Med' ? 'bg-amber-500' : 'bg-red-500'
            )} />
            {density} Density
          </span>
        </div>
...
function Step({ icon: Icon, text, time, highlight }: any) {
  return (
    <div className="flex items-start gap-4 p-2">
      ...
      <div className="flex-1">
        <div className={cn("text-sm font-medium", highlight ? "text-[#ccff00]" : "text-white")}>{text}</div>
