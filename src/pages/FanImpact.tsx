...
import { Droplets, Wind, Footprints, ChevronRight, CheckCircle2, Leaf } from 'lucide-react';
...
export default function FanImpact() {
  ...
  const data = [
    ...
    {
      name: 'Carbon',
      value: Math.round(sustainability.carbonSaved * 10),
      goal: 20,
      unit: 'kg x10',
    },
    {
      name: 'Walk',
      value: Math.round(sustainability.stepsTaken / 1000),
      goal: 10,
      unit: 'Steps (k)',
    },
  ];
    ...
    <div className="max-w-5xl mx-auto space-y-8 lg:space-y-12 pb-20">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-gray-500 max-w-xl mx-auto text-sm lg:text-base px-4">
          Every step, refill, and transit choice counts. See how your journey is contributing to the
          tournament's net-zero mission.
        </p>
      </div>
      ...
      >
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center relative z-10">
          <div className="text-center lg:text-left">
            <div className="text-[10px] font-bold text-[#ccff00] uppercase tracking-[0.3em] mb-4">
              Current Score
            </div>
            <div className="text-6xl lg:text-8xl font-black tracking-tighter leading-none mb-6">
              {sustainability.points}
              <span className="text-[#ccff00] text-xl lg:text-3xl ml-2">pts</span>
            </div>
            <div className="space-y-4 flex flex-col items-center lg:items-start">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-4 h-4 lg:w-5 lg:h-5 text-[#ccff00] shrink-0" />
                <span className="text-gray-300 text-sm lg:text-base">
                  Top 15% of sustainable fans today
                </span>
              </div>
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-4 h-4 lg:w-5 lg:h-5 text-[#ccff00] shrink-0" />
                <span className="text-gray-300 text-sm lg:text-base">
                  Net-Zero Contributor Badge unlocked
                </span>
              </div>
            </div>
          </div>
              ...
              <BarChart data={data}>
                ...
                <Bar dataKey="value" radius={[10, 10, 0, 0]}>
                  {data.map((_, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={index === 0 ? '#3b82f6' : index === 1 ? '#ccff00' : '#10b981'}
                    />
                  ))}
                </Bar>
              </BarChart>
