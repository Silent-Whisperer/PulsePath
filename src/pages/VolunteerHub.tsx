...
export default function VolunteerHub() {
  ...
  const tasks = [
    { id: 't1', title: 'Assist Wheelchair User', loc: 'Gate C', time: '5m ago', priority: 'high', type: 'assistance' },
    { id: 't2', title: 'Guide Fans to Alt Restroom', loc: 'Zone 4', time: '12m ago', priority: 'medium', type: 'operations' },
    { id: 't3', title: 'Water Station Refill Confirmation', loc: 'Sec 112', time: '20m ago', priority: 'low', type: 'safety' },
  ];
    ...
    <div className="max-w-2xl mx-auto space-y-8 pb-20">
      ...
      <div className="bg-gradient-to-br from-[#ccff00]/20 to-transparent border border-[#ccff00]/20 rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
        ...
        <div className="text-center sm:text-right w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-white/5">
          ...
          <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Karma Points</div>
        </div>
      </div>
      ...
      <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
        <button 
          ...
          className={cn(
            ...
            activeTab === 'tasks' ? "bg-[#ccff00] text-black shadow-lg" : "text-gray-500 hover:text-white"
          )}
        >
          Active Tasks
        </button>
        <button 
          ...
          className={cn(
            "flex-1 py-3 rounded-xl text-sm font-bold transition-all",
            activeTab === 'phrases' ? "bg-[#ccff00] text-black shadow-lg" : "text-gray-500 hover:text-white"
          )}
        >
          Multilingual Helper
        </button>
          ...
          <>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 border-dashed">
              <div className="flex items-center gap-4 text-gray-500">
                ...
                <Button variant="outline" className="ml-auto border-white/10 rounded-xl">Request</Button>
              </div>
            </div>
          </>
        ) : (
          <div className="grid grid-cols-1 gap-4">
            <PhraseCard phrase="Where is the nearest exit?" translations={{ es: "¿Dónde está la salida más cercana?", fr: "Où est la sortie la plus proche?" }} />
            <PhraseCard phrase="I need medical assistance." translations={{ es: "Necesito asistencia médica.", ar: "أحتاج إلى مساعدة طبية" }} />
            <PhraseCard phrase="Follow me to the accessible gate." translations={{ es: "Sígame a la puerta accesible.", hi: "सुलभ गेट के लिए मेरा अनुसरण करें" }} />
            <PhraseCard phrase="The match starts in 10 minutes." translations={{ es: "El partido comienza en 10 minutos.", fr: "Le match commence dans 10 minutes." }} />
          </div>
        )}
      </motion.div>
    </div>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-3xl p-6 hover:border-[#ccff00]/30 transition-all cursor-pointer group">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            {task.type === 'assistance' ? <Users className="w-5 h-5" /> : <Zap className="w-5 h-5" />}
          </div>
          <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <Button className="flex-1 bg-white/10 hover:bg-white/20 text-white rounded-xl py-4 lg:py-5 font-bold text-sm">View Location</Button>
        <Button className="flex-1 bg-[#ccff00] text-black hover:bg-[#d9ff33] rounded-xl py-4 lg:py-5 font-bold text-sm">Accept Task</Button>
      </div>
    </div>
  );
}
